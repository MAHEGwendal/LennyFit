<template>
  <section
    ...
    <div class="absolute inset-0 z-0">
      <img src="/hero/hero3.jpg" alt="" class="w-full h-full object-cover" fetchpriority="high"/>
    ...
    <div class="relative z-10 flex flex-col items-center mx-auto w-full space-y-6">
      <h1 class="font-krona text-3xl md:text-4xl font-medium leading-snug text-icemint">
        Un coaching adapté à vos <span class="text-molten">besoins réels</span>, votre <span
          class="text-molten">santé</span> et vos <span class="text-molten">capacités</span>
      </h1>
    </div>
  </section>
  <div class="max-w-7xl mx-auto mt-10 mb-10 px-4">
    <div class="flex flex-col items-stretch gap-8 lg:gap-20">

      <div v-for="categorie in categories" :key="categorie.title"
        class="flex-1 flex flex-col rounded-2xl shadow-xl/30 py-8 px-4 border-2 border-black/10">
        <h3 class="font-krona text-2xl lg:text-4xl mb-5 text-ink text-center">{{ categorie.title }}</h3>
  ...
  <div class="px-4 py-8 md:py-12 max-w-4xl mx-auto w-full">
    <div class="py-8 px-4 md:py-12 md:px-8 rounded-xl shadow-xl/30 bg-transparent">
      ...
      <div class="flex justify-center">
        <RouterLink to="/Formules" role="button" aria-label="Les formules"
          class="inline-block bg-molten border-3 border-transparent text-ink font-zalando px-8 py-5 rounded-xl font-medium text-2xl shadow-xl/20 md:px-7 md:py-4 md:text-2xl transform hover:scale-110 transition">
          Les formules
        </RouterLink>
      </div>
    </div>
...
const categories = [
  {
    ...
    description: "Utiliser l'activité physique comme un moyen d'améliorer durablement la santé, l'équilibre de vie et le bien-être physique et mental, à travers une pratique adaptée, progressive et accessible à chacun.",
    services: [
      {
